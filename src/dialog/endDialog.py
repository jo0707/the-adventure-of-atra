...
class EndDialog(pygame.sprite.Group):
    def __init__(self, x: int, y: int, onClose = lambda: print("Close item detail clicked"), quizzez: List[Quiz] = []):
        ...
        totalCorrectAnswer = self.player.lampungCorrectAnswers + self.player.sumateraBaratCorrectAnswers + self.player.sumateraUtaraCorrectAnswers
        totalQuestion = 11
        
        quizDoneTextbox = Textbox("Kuis selesai!", 0, 0, 32, (0, 0, 0))
        correctAnswerTextbox = Textbox(f"Jawaban benar kamu ada {totalCorrectAnswer} dari {totalQuestion}", 0, 0, 24, (45, 134, 45))
        ...
        self.okButton = Button(x + self.pad, self.height // 2 + 200, 896, 75, "Keren!", action=onClose)
        self.add(self.background, self.okButton)
